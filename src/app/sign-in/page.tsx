...
export default function SignInPage() {
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-10">
      ...
      <form onSubmit={onPassword} className="mb-6 grid gap-3">
        <input
          type="email"
          required
          value={email}
        ...
        <input
          ...
          className="w-full rounded-lg border px-3 py-2"
        />
        <button disabled={status==='sending'} className="rounded-xl btn-primary px-4 py-2 disabled:opacity-60">
      ...
      <div className="flex gap-3">
        <button onClick={() => onOAuth('github')} className="rounded-lg border px-3 py-1.5 hover:bg-gray-50">Continue with GitHub</button>
        <button onClick={() => onOAuth('google')} className="rounded-lg border px-3 py-1.5 hover:bg-gray-50">Google</button>
      </div>
